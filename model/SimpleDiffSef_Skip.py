...
class CondGaussianDiffusion(GaussianDiffusion):
    ...
    def p_mean_variance(self, x, cond, extra_cond, time, time_next):
        ...
        if extra_cond is None:
            pred = self.model.sample_unet(torch.cat([x], dim=1),
                                          batch_log_snr, cond)
        else:
            if len(self.feats) % 2 == 0:
                feat = self.model.backbone(torch.cat([x, extra_cond], dim=1), batch_log_snr, cond)
            else:
                feat = self.feats[-1]
            self.feats.append(feat)
            pred, _, _, _, _ = self.model.decode_head(feat, batch_log_snr, torch.cat([x, extra_cond], dim=1))
